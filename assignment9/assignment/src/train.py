"""
Budget-optimized ResNet50 training for 77%+ accuracy on ImageNet
...
def train_epoch(model, train_loader, criterion, optimizer, scaler, epoch, device, mixup=None):
    """Training epoch with mixed precision and mixup"""
...
def main():
    ...
    data_module = ImageNetDataModule(args.data_dir, batch_size=batch_size, num_workers=num_workers)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        train_acc1, train_loss = train_epoch(
            model, train_loader, criterion, optimizer, scaler, epoch, device, mixup
        )
        ...
        if epoch % 5 == 0 or epoch == epochs - 1:
            ...
            estimated_cost = hours_elapsed * 0.526
            log_and_print(f'  Est_Cost: ${estimated_cost:.2f}')
            ...
            if epoch >= 30 and best_acc1 < 65.0:
                log_and_print(f"WARNING: Low accuracy at epoch {epoch+1}: {best_acc1:.3f}%")
                log_and_print("Consider adjusting learning rate or check for issues")
            elif epoch >= 50 and best_acc1 < 70.0:
                log_and_print(f"WARNING: May not reach 75%+ target")
            ...
            if estimated_cost > 16.0:
                log_and_print(f"💸 BUDGET WARNING: Cost ${estimated_cost:.2f} approaching $18 limit")
                if best_acc1 >= 73.0:
